...
if __name__ == "__main__":
    args = sys.argv[1:]
    if not args:
        print("Usage:", os.path.basename(sys.argv[0]), "<first.fontspec> [<second.fontspec>...]")
        sys.exit(2)
    try:
        args.remove("-v")
        verbose = True
    ...
    for specfile in args:
        ...
        with open(specfile, encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if m:
                    imgfile = os.path.join(os.path.dirname(specfile), m.group(1))
                    if verbose: print("  - loading image:", imgfile, f" ({m.group(2)}x{m.group(3)})")
                    try:
                        img = SourceImage(imgfile, coord(int(m.group(2)), int(m.group(3))))
                        if verbose: print("    - contains", len(img.glyphs), "glyph bitmaps")
                    except EnvironmentError as e:
                        err(f"can not import '{imgfile}': {e}")
                        img = None
                    continue
    ...
    with open("retrofont/src/fonts.c", "w") as out:
